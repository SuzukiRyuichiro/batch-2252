...
until item == 'quit'
  ...
  if store.key?(item)
    ...
    # Check if the requested amount is more than available at the store, send them back to the top of the loop
    if amount > store[item][:stock]
      puts "Sorry we don't have enough of that"
      next
    end

    # if not, continue with the existing logic
    ...
    # update the available amount in the store.
...
cart.each do |item, amount|
  ...
  # e.g. kiwi: 2 X 1.25€ = 2.5€
  puts "#{item}: #{amount} X #{store[item][:price]}€ = #{subtotal}€"
  total += subtotal # increment the total by subtotal
end
